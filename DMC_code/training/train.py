...
def train(train_loader, model, criterion, optimizer, epoch, args, tensor_writer=None, mi_upper_estimator=None, dow=None):
    ...
    for i, (input_ids, attention_masks, segment_ids, target) in enumerate(train_loader):
        ...
        output, cfeatures, hidden_states = model(input_ids, attention_masks, segment_ids) #output x, flatten_features(CLS的token（hidden_states[0]）再过一下全连接层+Tanh激活函数，作为该句子的特征向量), hidden_states（bert输出的所有的token） //cls hiddenstates vectors cfeatures ? 
		...
		# 找到属于当前类别的向量的索引
                        ...
                        if len(indices) > 0:
                                ...
                                with open(f"/home/user/class/3/class_{i}.npy", "ab") as f:
