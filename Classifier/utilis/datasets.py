...
def processSentences(tokenizer, samples1_list):
    input_ids = []
    attention_masks = []
    segment_ids = []
    ...
    for idx in range(len(samples1_list)):
        ...
        encoding = tokenizer.encode_plus(sent1, max_length=SENTENCE_MAX_LENGTH, truncation=True)   #cut length of input as 512 directly
...
class datasets(Dataset):
    ...
    def __init__(self, datadir, tokenizer, labels_type, args):
        ...
        #self.samples2_list = []
        self.samples_labels = []
        #self.label2id = {label: i for i, label in enumerate(labels_type)} #['cat', 'dog', 'bird'], the resulting dictionary would be {'cat': 0, 'dog': 1, 'bird': 2}
        with open(datadir,'r') as f:
            for line in f:
                word_list = line.split()    #eg: hash mov add ... mov 3
                self.samples_labels.append(int(word_list[-1]))
                self.samples1_list.append(" ".join(word_list[1:-1]))
        ...
        input_ids, attention_masks, segment_ids = processSentences(tokenizer, self.samples1_list)
...
class datasets2(Dataset):
    ...
    def __init__(self, feature, label, tokenizer, labels_type, args):
        #df_samples = pd.read_csv(datadir, sep='\t', engine='python', quoting=csv.QUOTE_NONE)

        self.samples1_list = feature
        #self.samples2_list = []
        self.samples_labels = label
        #self.label2id = {label: i for i, label in enumerate(labels_type)} #['cat', 'dog', 'bird'], the resulting dictionary would be {'cat': 0, 'dog': 1, 'bird': 2}
        # with open(datadir,'r') as f:
        #     for line in f:
        #         word_list = line.split()    #eg: hash mov add ... mov 3
